...
include 'conexion.php'; // Asegúrate de que esta ruta sea correcta para tu proyecto
...
switch ($accion) {
    case 'crear_mensaje':
        ...
        $fecha_publicacion = date('Y-m-d H:i:s'); // Fecha y hora actual AUTOMÁTICA
    ...
    case 'listar_mensajes':
        // Listar todos los mensajes (para el preceptor), incluyendo fecha_publicacion
        $sql = "SELECT 
                    idMensaje, 
                    titulo, 
                    mensaje_texto, 
    ...
    case 'obtener_mensajes_activos': // Para los módulos de Profesor y Alumno (ya lo tienes, lo incluyo por completitud)
        ...
        $sql = "SELECT 
                    ...
                    fecha_publicacion 
                FROM cartelera 
                WHERE (fecha_expiracion IS NULL OR fecha_expiracion >= CURDATE())"; 
    ...
    case 'actualizar_mensaje':
        ...
        $publico_destino = $_POST['publico_destino'] ?? '';
        $fecha_expiracion = $_POST['fecha_expiracion'] ?? null;
        // La fecha de publicación no se actualiza aquí, se mantiene la original.
